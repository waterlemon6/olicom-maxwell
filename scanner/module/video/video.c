...
static long	video_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case VIDEO_PORT_WRITE_REG:
			__get_user(temp, (__u32 __user *)arg);
			spi_reg_write_word(temp);
			break;
		case VIDEO_PORT_GET_IMAGE_OFFSET:
			status = offsetInPage;
			break;
		case VIDEO_PORT_RESUME:
			bsp_csi_init();
			bsp_csi_enable_module();
			flush_cache_all();
			MOSI_DATA_RESET;
...
{
	printk(KERN_ALERT "[video]""Hello world.\n");
	...
	memset(imageBuf, 80, VIDEO_PORT_WIDTH * VIDEO_PORT_HEIGHT);
...
{
	printk(KERN_ALERT "[video]""Goodbye, cruel world.\n");
...
MODULE_DESCRIPTION("video of waterlemon");
MODULE_LICENSE("GPL");
