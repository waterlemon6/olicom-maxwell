...
static int usblp_ctrl_msg(struct usblp *usblp, int request, int type, int dir, int recip, int value, void *buf, int len)
{
	...
	retval = usb_control_msg(usblp->dev, dir ? usb_rcvctrlpipe(usblp->dev, 0) : usb_sndctrlpipe(usblp->dev, 0),
		request, type | dir | recip, value, index, buf, len, USBLP_CTL_TIMEOUT);
	dev_dbg(&usblp->intf->dev, "usblp_control_msg: rq: 0x%02x dir: %d recip: %d value: %d idx: %d len: %#x result: %d\n",
...
static void usblp_bulk_write(struct urb *urb)
{
	...
	if (usblp->present && usblp->used) {
		if (status)
			printk(KERN_WARNING "usblp%d: nonzero write bulk status received: %d\n",
				usblp->minor, status);
	}
...
static int usblp_check_status(struct usblp *usblp, int err)
{
	...
	if ((error = usblp_read_status(usblp, usblp->statusbuf)) < 0) {
		...
		printk_ratelimited(KERN_ERR "usblp%d: error %d reading printer status\n",
				usblp->minor, error);
		return 0;
	}
...
static long usblp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	dev_dbg(&usblp->intf->dev, "usblp_ioctl: cmd=0x%x (%c nr=%d len=%d dir=%d)\n", cmd,
	...
	if (_IOC_TYPE(cmd) == 'P')	/* new-style ioctl number */
		switch (_IOC_NR(cmd)) {
			case IOCNR_GET_DEVICE_ID: /* get the DEVICE_ID string */
				...
				if (copy_to_user((void __user *) arg, usblp->device_id_string, (unsigned long) length)) {
					retval = -EFAULT;
					goto done;
				}
				break;
			...
			case IOCNR_GET_PROTOCOLS:
				if (_IOC_DIR(cmd) != _IOC_READ ||  _IOC_SIZE(cmd) < sizeof(twoints)) {
					retval = -EINVAL;
					goto done;
				}
				...
				twoints[1] = 0;
				for (i = USBLP_FIRST_PROTOCOL; i <= USBLP_LAST_PROTOCOL; i++) {
					if (usblp->protocol[i].alt_setting >= 0)
						twoints[1] |= (1<<i);
				}

				if (copy_to_user((void __user *)arg, (unsigned char *)twoints, sizeof(twoints))) {
					retval = -EFAULT;
					goto done;
				}
				break;
			...
			case IOCNR_SET_PROTOCOL:
				...
				if (retval < 0) {
					usblp_set_protocol(usblp, usblp->current_protocol);
				}
				break;
			...
			case IOCNR_GET_BUS_ADDRESS:
				...
				dev_dbg(&usblp->intf->dev, "usblp%d is bus=%d, device=%d\n",
			...
			case IOCNR_GET_VID_PID:
				...
				dev_dbg(&usblp->intf->dev, "usblp%d is VID=0x%4.4X, PID=0x%4.4X\n",
					usblp->minor, twoints[0], twoints[1]);
				break;
			...
			case IOCNR_GET_MANUFACTURER_STRING:
				string = kmalloc(USBLP_STRING_SIZE, GFP_KERNEL);
...
static struct urb *usblp_new_writeurb(struct usblp *usblp, int transfer_length)
{
	...
	usb_fill_bulk_urb(urb, usblp->dev, usb_sndbulkpipe(usblp->dev,
...
static ssize_t usblp_read(struct file *file, char __user *buffer, size_t len, loff_t *ppos)
{
	...
	if (count != 0 && copy_to_user(buffer, usblp->readbuf + usblp->readcount, count)) {
		count = -EFAULT;
		goto done;
	}
...
static ssize_t usblp_show_ieee1284_id(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (usblp->device_id_string[0] == 0 && usblp->device_id_string[1] == 0)
		return 0;
...
static int usblp_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	usblp->quirks = usblp_quirks(le16_to_cpu(dev->descriptor.idVendor),
		le16_to_cpu(dev->descriptor.idProduct));

	/* Analyze and pick initial alternate settings and endpoints. */
	protocol = usblp_select_alts(usblp);
	if (protocol < 0) {
		dev_dbg(&intf->dev, "incompatible printer-class device 0x%4.4X/0x%4.4X\n",
	...
	dev_info(&intf->dev,
		...
		usblp->ifnum, usblp->protocol[usblp->current_protocol].alt_setting, usblp->current_protocol,
		le16_to_cpu(usblp->dev->descriptor.idVendor), le16_to_cpu(usblp->dev->descriptor.idProduct));
...
static int usblp_select_alts(struct usblp *usblp)
{
	...
	for (i = 0; i < if_alt->num_altsetting; i++) {
		...
		usblp->protocol[ifd->desc.bInterfaceProtocol].alt_setting = ifd->desc.bAlternateSetting;
		usblp->protocol[ifd->desc.bInterfaceProtocol].epwrite = epwrite;
		usblp->protocol[ifd->desc.bInterfaceProtocol].epread = epread;
	}

	/* If our requested protocol is supported, then use it. */
	if (proto_bias >= USBLP_FIRST_PROTOCOL && proto_bias <= USBLP_LAST_PROTOCOL &&
...
MODULE_DESCRIPTION("printer host of waterlemon");
