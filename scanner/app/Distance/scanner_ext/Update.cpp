...
const unsigned int crc32Init = 0xffffffff;
...
unsigned int GetCRC32CheckSum(unsigned char *pchMessage, unsigned int dwLength) {
    unsigned int ucCRC32 = crc32Init;
    while (dwLength--) {
        ucCRC32 = (ucCRC32 << 8) ^ crc32Tab[((ucCRC32 >> 24) ^ *pchMessage) & 255];
        pchMessage++;
    }
    return ucCRC32;
}
...
bool Update() {
    ...
    const int cut = 8000;
    auto *buffer = new unsigned char [cut];
    ...
    fwrite(buffer, (size_t)bytes, 1, stream);
    count -= bytes;

    printf("Prepare to download xxx.img, length: %d.\n", length);
    while (count > 0) {
        if (printerDevice.Poll(POLLIN | POLLRDNORM, 1000)) {
            bytes = printerDevice.Read(buffer, cut);
            if (bytes > 0) {
                fwrite(buffer, (size_t)bytes, 1, stream);
                count -= bytes;
            }
    ...
    printf("Prepare to decompress xxx.img.\n");
    ...
    fread(update, length, 1, stream);
    UpdateDecompress(update, length);
    fclose(stream);
